...
The repo set is defined **exclusively** by the `kind: project` manifest's `repositories:` list. `kind: service` (build/execute an app) and `kind: stateful` (backing dep) manifests never alter which repos belong to the workspace. A repo belongs to the workspace because the project manifest lists it; a repo may contain zero, one, or many service manifests.
