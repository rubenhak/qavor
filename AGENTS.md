...
**Profile resolution happens at registry-build time.** When `buildWorkspaceRegistry` assembles the registry, every entry's `profiles:` chain is flattened into its `runtime`/`mode`/`env` (later profiles and the entry's own values winning; chained profiles supported — a profile may itself carry a `profiles:` list) and the `profiles:` key is dropped. Every command (`prepare`, `up`, `env`, …) therefore reads the *effective* definition from `entry.data` without re-resolving. The standalone `resolve-manifest` command is purely a debug printer over that already-resolved data. Profile cycles and unknown profile references are reported as manifest issues. Keep the flattening logic in `manifest/resolve.ts` as the single implementation.

By default a merged value **replaces** its inherited counterpart for scalars and arrays (`mode`, and each command's `operations` such as `prepare`'s); `env` maps deep-merge key by key. Every runtime command shares one uniform shape — `{ description?, operations }` (see `runtimeDescribedCommand` in the schema) — with no structural difference between the reserved keys (`check_installed`, `install`, `run`) and user-defined commands; `description` is documentation-only and `operations` holds the step(s). A command's `operations` additionally accepts a **profile-merge directive** — an object with exactly one of `$append` / `$prepend` / `$replace` / `$unset` — so a child can extend rather than replace an inherited step list (`$replace` is the explicit default; `operations: { $unset: true }` drops the inherited command entirely). Directives apply to every command, including `run` (whose result must still resolve to a single step). They are resolved inside `deepMerge`/`applyDirective` in `manifest/resolve.ts` against the already-accumulated base, so they compose down a chain; a command whose merged `operations` is `$unset` is dropped via `isCommandUnset`. Reuse `normalizeSteps` (`manifest/steps.ts`) and `normalizeCommandValue` (`manifest/runtime.ts`, which unwraps `operations`) rather than re-implementing single-object↔list coercion.
