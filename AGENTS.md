...
manifest commands (including `run` — qavor ships no built-in process
supervisor; `up`/`down`/`logs`/`ps` were removed and a long-running service is
just a foreground `qavor run`), env composition (incl. `require:` deps and
`env.publish` contracts), and profiles (local + remote, chaining, merge
directives). Not yet built: container /
...
|---|---|---|
| `workspaces` | Pointer to the project repo dir. **Multi-repo only** — generated by `qavor init`; single-repo (`standalone`) projects have none. | Workspace-root `qavor.yaml`. |
...
**Profile resolution happens at registry-build time.** When `buildWorkspaceRegistry` assembles the registry, every entry's `profiles:` chain is flattened into its `runtime`/`mode`/`env` (later profiles and the entry's own values winning; chained profiles supported — a profile may itself carry a `profiles:` list) and the `profiles:` key is dropped. Every command (`prepare`, `run`, `env`, …) therefore reads the *effective* definition from `entry.data` without re-resolving. The standalone `resolve-manifest` command is purely a debug printer over that already-resolved data. Profile cycles and unknown profile references are reported as manifest issues. Keep the flattening logic in `manifest/resolve.ts` as the single implementation.

By default a merged value **replaces** its inherited counterpart for scalars and arrays (`mode`, and each command's `operations` such as `prepare`'s); `env` maps deep-merge key by key. Every runtime command shares one uniform shape — `{ description?, operations }` (see `runtimeDescribedCommand` in the schema) — with no structural difference between the two reserved keys (`check_installed`, `install`) and user-defined commands; `description` is documentation-only and `operations` holds the step(s). `run` is **not** reserved — it is an ordinary user-defined command like `prepare`/`lint`, with no special supervision. A command's `operations` additionally accepts a **profile-merge directive** — an object with exactly one of `$append` / `$prepend` / `$replace` / `$unset` — so a child can extend rather than replace an inherited step list (`$replace` is the explicit default; `operations: { $unset: true }` drops the inherited command entirely). Directives apply to every command without exception. They are resolved inside `deepMerge`/`applyDirective` in `manifest/resolve.ts` against the already-accumulated base, so they compose down a chain; a command whose merged `operations` is `$unset` is dropped via `isCommandUnset`. Reuse `normalizeSteps` (`manifest/steps.ts`) and `normalizeCommandValue` (`manifest/runtime.ts`, which unwraps `operations`) rather than re-implementing single-object↔list coercion.
...
**Remote profile directory sources (ADR-009).** A source path that does not end in `.yaml`/`.yml` is a **directory reference**: the profile is `<dir>/qavor.yaml` and the entire directory (compose files, configs) is materialized locally — in place for `file://`, via the whole clone for git, via the tree API + raw downloads (capped 100 files / 10 MB) for GitHub. Plain https directory refs fail closed. Registry entries keep `dir` pointing at the materialized directory so step-relative paths resolve there.

**Declarative runtime steps (ADR-008).** A step in a command's `operations` is a keyed union: a bare shell `cmd` step (unchanged), or a `compose:` / `docker:` step (`manifest/steps.ts` guards; executors in `src/command/{compose-step,docker-step}.ts`). Declarative steps interpolate `${VAR}` in their fields from the composed env (fail-closed, `src/command/interpolate.ts`) and shell out to the docker CLI via execa — never reimplement docker. Profile flattening stamps every profile-contributed step with an internal `$dir` origin annotation (`resolve.ts`, `STEP_ORIGIN_KEY`) so profile-shipped **asset** paths (`file:`/`env_file:`) resolve against the defining manifest's directory (a remote profile's materialized dir); each step also sees `QAVOR_MANIFEST_DIR` set to that dir. A step's **working directory is not** the origin dir — `cwd` (and the default cwd) resolves against the consuming service's directory, so referencing a profile behaves exactly like copying its steps inline (a step runs and writes output where the service lives, not in the profile's cache dir). The executor keeps the two dirs distinct via `DeclarativeStepContext.serviceDir` (cwd) vs `.originDir` (assets); see `src/command/command.ts`. Strip the annotation with `stripStepOrigins` before dumping manifests to users. When touching step semantics, update `docs/schemas/qavor.defs.schema.json` first (both copies — `src/schema/` is the hand-synced runtime copy), regen types, and keep `test/declarative-steps.test.ts` green.

**Service library (`library/`).** First-party backing-service templates (postgresql, mysql, redisearch, kind), each a self-contained directory: multi-document `qavor.yaml` (profile `lib-<dir>` first, ready `kind: service` second) plus sibling files, consumed via ADR-009 directory sources. Binding conventions (guarded by `test/library.test.ts`): uniform verbs `up/down/status/logs/purge` + `check_installed`/`install`, idempotent `up` that blocks until ready, non-destructive `down` (data wipes only in `purge`), every parameter a long-form envSpec with `description` plus either `default` or `cmd` (a dynamically-derived value, resolved after every static env — see §7) (secrets marked), per-template env prefixes, pinned image tags, POSIX `/bin/sh` in `cmd` steps, `env.publish` contract for dependents. Template interface changes (env names, publish keys, verbs) are breaking for pinned consumers. Full conventions in `library/README.md`.
...
**Dynamic env (`cmd`).** An `envSpec` entry may carry `cmd` (a shell script) instead of `value`/`default` — mutually exclusive, schema-enforced. `cmd` entries resolve in a second phase, strictly after every static entry above (1–4) is fully resolved, in original layer declaration order; a `cmd` entry overwrites any static (or earlier-`cmd`) value for the same key, and later `cmd` entries see earlier ones' output. The script runs via `/bin/sh -c` (override with `shell`), cwd the defining manifest's directory, with an env of `process.env` plus everything resolved so far — so it reads other vars as ordinary shell variables (`$FOO`), not qavor's `${VAR}` interpolation. stdout, trimmed, becomes the value. A non-zero exit or spawn failure throws `RuntimeFailure` (exit 3 — a subprocess failure, not a manifest defect) and aborts the whole composition; it does not fail closed into a collected `ManifestComposeIssue` like a missing/unresolved static var does. Implementation: `resolveDynamicLayers` in `src/env/composer.ts`.

---

## 8. Process supervision (ADR-002 — superseded)

- **`mode: native`** → **no built-in supervisor.** The native process-supervision layer (`up`/`down`/`logs`/`ps`, detached process groups, PID/state tracking) has been **removed**: qavor hard-codes no command names and does not daemonize. A long-running service is expressed as an ordinary manifest command (`run`, by convention) and run in the foreground via `qavor run`; backgrounding/signalling is the command's own responsibility (nohup, a process manager, `docker compose up -d`, …). This supersedes the original ADR-002 native-supervisor decision.
...
├── supervisor/         # (removed) native process supervisor — qavor no longer daemonizes; compose driver is post-MVP
...
- `qavor init`, `qavor doctor`, `qavor clone`, `qavor sync`, `qavor status`, `qavor commit`, `qavor push`, `qavor <command>` (any manifest command, including `run`), `qavor env <service>`, `qavor validate`, `qavor workspace info`. (`up`/`down`/`logs`/`ps` were removed — no built-in supervisor.)
- **Dynamic commands.** A runtime backend's keys other than the reserved `enabled` / `check_installed` / `install` are user-defined commands (`run`, `prepare`, `update_libraries`, `lint`, …). qavor discovers them across the workspace at startup and registers one `qavor <command>` subcommand each, fanning the command out over the services that declare it (`runtime.native.<command>`). Every command uses one uniform shape, `{ description?, operations }` — `operations` is a single step, a list of steps, or a profile-merge directive, and `description` is an optional documentation-only string surfaced by `qavor commands` and by `qavor <command> --help`'s one-line summary; it has no effect on execution and merges like any other backend key when a service references a profile (see `manifest/runtime.ts`'s `serviceCommandDescription` / `serviceCommandSteps`, which unwrap `operations`). `qavor commands` (and its `--json` form) lists every command, its description, and its declaring services — this is also the primary surface a skill uses to discover what a workspace can run. No command set is hard-coded; the runner lives in `src/command/` and the registration in `src/cli/commands/dynamic.ts`. The `pre_command` / `post_command` hooks fire around each run with `QAVOR_COMMAND` set.
- Native mode only. No graph orchestration yet.
