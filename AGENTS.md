...
By default a merged value **replaces** its inherited counterpart for scalars and arrays (`mode`, and each runtime step list such as `prepare`); `env` maps deep-merge key by key. Runtime step-list fields additionally accept a **profile-merge directive** — an object with exactly one of `$append` / `$prepend` / `$replace` / `$unset` — so a child can extend rather than replace an inherited step list (`$replace` is the explicit default; `$unset: true` drops the inherited command). Directives apply to `check_installed`, `install`, and user-defined commands, but not `run` (single-step). They are resolved inside `deepMerge`/`applyDirective` in `manifest/resolve.ts` against the already-accumulated base, so they compose down a chain. Reuse `normalizeSteps` (`manifest/steps.ts`) rather than re-implementing single-object↔list coercion.
