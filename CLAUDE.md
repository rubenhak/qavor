...
- **Declarative steps (ADR-008/009)**: a step is `cmd:` (shell) OR `compose:`/`docker:` (declarative, `${VAR}`-interpolated fail-closed, executed via the docker CLI). Profile-contributed steps carry an internal `$dir` origin so profile-shipped **assets** (`file:`/`env_file:`, `$QAVOR_MANIFEST_DIR`) resolve against the profile's own (possibly remotely materialized) directory — but a step's **cwd** resolves against the consuming service's dir (copy-inline semantics), not the profile's; a `profiles:` source path without a `.yaml` suffix fetches the whole directory. See `AGENTS.md §6` and `library/README.md`.
