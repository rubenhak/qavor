...
import { updateServiceLibraries } from '../../update-libraries/update-libraries.js';
...
import { createUpdateLibrariesView, type UpdateLibrariesRow } from './update-libraries-view.js';

export function registerUpdateLibraries(program: Command): void {
  program
    .command('update-libraries')
    .description('Run `runtime.native.update_libraries` for every selected service.')
    ...
    .action(async (opts: { only?: string[]; env?: string[] }, cmd: Command) => {
      ...
      // until its update resolves. Auto-disabled for --json, non-TTY, and
      // --verbose (where the command's raw output shares the terminal); those
      // fall back to a single static render in `finish`.
      const view = createUpdateLibrariesView(
        services.map((s) => s.name),
        { enabled: !root.json && !root.verbose },
      );
      const rows = await runFanOut(
        services,
        async (entry, index): Promise<UpdateLibrariesRow> => {
          ...
          const updateOpts: Parameters<typeof updateServiceLibraries>[0] = {
            paths: ws.paths,
            serviceDoc,
            service,
            logger,
          ...
          if (cliEnv) updateOpts.cliEnv = cliEnv;
          let row: UpdateLibrariesRow;
          try {
            const res = await updateServiceLibraries(updateOpts);
            row =
              res.status === 'ok'
                ? { service: res.serviceName, outcome: 'ran', status: 'updated' }
                : {
                    service: res.serviceName,
                    outcome: 'skip',
                    status: 'no update',
                    detail: 'no update_libraries command',
                  };
          } catch (err) {
            row = {
              service: entry.name,
      ...
      if (root.json) {
        // Machine contract: `serviceName` plus the underlying `ok` /
        // `no-update-cmd` status, with `failed` for errored services.
        emitJson({
          results: rows.map((r) => ({
            serviceName: r.service,
            status: r.outcome === 'ran' ? 'ok' : r.outcome === 'skip' ? 'no-update-cmd' : 'failed',
      ...
      if (rows.some((r) => r.outcome === 'fail')) {
        throw new RuntimeFailure('Some services failed to update libraries.');
      }
    });
}
