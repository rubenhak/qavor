...
- Uniform UX across git, dependency prep, stateful deps, native run, container build/run.
...
flowchart LR
    ...
    CLI --> RepoCfg["Repo / service / stateful / profile manifests"]
...
- **Project repo** — the seed git repo whose root holds the **project manifest** (`kind: project`). Defines the workspace identity and lists every repo in the workspace.
- **Repo manifest** (`kind: repo`) — minimal per-repo metadata at the root of an individual repo: identity, group memberships, repo-level lifecycle hooks. Optional when the root manifest is already a `service` or `stateful` document.
- **Service manifest** (`kind: service`) — a runnable application. Lives at the repo root (single-service repo) or under a sub-directory (multi-service repo).
- **Stateful service manifest** (`kind: stateful`) — an externally provided backing dep (postgres, kafka, redis, …). v0 always delegated to `docker-compose` (per ADR-005).
- **Profile manifest** (`kind: profile`) — reusable runtime + env bundle, referenced from services and stateful manifests via a `profiles:` list.
- **Group** — labelled set of repos / services (e.g., `frontend`, `data`, `auth-stack`). A repo may belong to multiple.
- **Run mode** — `native` or `docker` per service; `docker-compose` for stateful services. Overridable per invocation via `--mode`.
...
- workspace-dir/                        # not a git repo
  ...
  - multi-service-repo.git/
    - qavor.yaml                        # kind: repo (optional)
    - service-foo/qavor.yaml            # kind: service
    - service-bar/qavor.yaml            # kind: service
  - stateful-deps.git/
    - qavor.yaml                        # kind: repo
    - postgresql/qavor.yaml             # kind: stateful
    - kafka/qavor.yaml                  # kind: stateful
```
...
### 5.3 Stateful Dependencies

- (existing) Bringup of postgres / mysql / kafka / etc.
- (new) **Backed by docker compose** under the hood — `qavor` generates and owns the compose project (per ADR-005) from `kind: stateful` documents.
- (new) **Versioned and pinned** — image versions live in the stateful manifest's `runtime.docker-compose.run.cmd` or in env (e.g. `POSTGRES_IMAGE: postgres:16.3`).
...
- (new) **Reset / recreate / snapshot / restore** — `qavor stateful reset postgres`, optional snapshot for fast rewinds.
- (new) **Auto port allocation** — avoid conflicts when multiple workspaces coexist.
- (new) **Multiple instances** — e.g., a `postgres-app` stateful and a separate `postgres-test` stateful manifest.
...
- (existing) Per-service and per-stateful-dep env vars
- (existing) YAML-defined env
- (existing) `.env` overrides
- (existing) Platform-specific (native vs docker)
- (new) **Layered env block** — every service / stateful / profile manifest has `env: { common, native, docker }`. `common` always applies; `native` or `docker` is layered on top depending on the active mode.
- (new) **Stateful publish contract** — stateful manifests add `env.publish:`, the only vars that flow to dependents.
- (new) **Explicit precedence** — defined in §6 below; identical for every service/stateful at every layer.
- (new) **Interpolation & references** — `${POSTGRES_HOST}`, `${secret:DB_PASSWORD}`. Cross-service references use the stateful `publish` map rather than ad-hoc lookups.
...
- (existing) Services declare dependencies on other services and stateful deps
- (existing) Env vars from deps propagate across the chain (via stateful `publish`)
...
- `pre`/`post` hooks on key verbs via the `hooks:` block on repo / service / stateful manifests.
...
- `qavor docs` renders the workspace catalog (repos, services, stateful deps, env contracts) as markdown — great for onboarding READMEs.
...
When the same env key is set in multiple places, **later layers win**. For a given service or stateful invocation in a given mode, qavor composes env top-down:

1. **Required dependencies** (from each required service / stateful, recursively):
   ...
   - For stateful deps, only `env.publish` is forwarded to dependents.
2. **The service or stateful itself**:
...
Profiles attached to a service / stateful are resolved before step 2 — each profile's own `env.common`/`env.native`/`env.docker` is layered in the order profiles are declared (later profiles win), and the manifest's own env wins over all of them.
...
**Stateful manifest** (under a sub-directory of a deps repo, or at a single-stateful repo root):
...
**Repo manifest** (when a repo wants metadata without being a service or stateful):
...
**Profile manifest** (referenced by services / stateful via `profiles:`):
...
- `qavor stateful up|down|reset|snapshot|restore`
...
- **ADR-002** Process supervision → own minimal native supervisor + compose for containers/stateful.
