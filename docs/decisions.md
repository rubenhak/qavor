...
**Consequences.**
- **Origin-dir tracking.** Steps flattened from a profile are stamped (internal `$dir` annotation, post-validation) with the profile's own directory; the *asset* paths `file:` and `env_file:` resolve against it, and each step sees `QAVOR_MANIFEST_DIR` so a profile can reach any file it ships. The step's **working directory is deliberately not** this dir: `cwd` (and the default cwd) resolves against the *consuming service's* directory, so referencing a profile behaves as if its steps were copied inline and a step runs/writes where the service lives — not in the profile's (possibly cache) directory. (This reverses an earlier iteration that resolved `cwd` against the profile dir too, which stranded a step's output in the materialized cache.)
